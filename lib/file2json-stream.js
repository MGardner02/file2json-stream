...
class file2jsonStream extends Transform {
    ...
    generateOptionSet(optionSet) {
        ...
        optionSet.forEach(option => {
            if (option.value && typeof option.value === 'string')
                ...
                set.fields.push('FIELD_' + (option.order !== undefined ? option.order : set.fields.length + 1))
    ...
    updateObject(obj, value, field, scale) {
        if (scale !== undefined && scale >= 0) {
            if (!isNaN(value))
                obj[field] = Number(value) / Math.pow(10, scale)
        } else {
            obj[field] = value
        }
        return obj
            ...
            for (var i = 0; i < optionSet.fields.length; i++) {
                ...
                if (optionSet.regex[i]) {
                    if (value.match(optionSet.regex[i])) {
                        this.updateObject(obj, value, optionSet.fields[i], optionSet.scale[i])
                    }
                } else {
                    this.updateObject(obj, value, optionSet.fields[i], optionSet.scale[i])
                }
