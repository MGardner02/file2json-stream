 ...
 *  scale:
 * },...]
 *  headerRequired: false, // abort if missing
 *  footerRequired: false // abort if missing
 * }
 */
...
class text2json extends Transform {
  constructor (options, transformOptions) {
    ...
    if (options.header) { this.header = this.generateOptionSet(options.header) }
    if (options.footer) { this.footer = this.generateOptionSet(options.footer) }
  ...
  _flush (callback) {
    if (this.footer && !this._footerFound && this.options.footerRequired) { this.emit('error', new Error('Missing Footer')) }
    callback()
  }
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (this._lineCount === 1 && this.header) {
        try {
          ...
          if (this.options.headerRequired) { this.emit('error', new Error('Invalid Header')) }
        }
      } else {
        try {
